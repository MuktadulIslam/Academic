...
float getInformationGain(vector<dataItem> edge, int attributeNo) {
    float prov_Ci_Xi;
    float prov_Ci[TOTAL_CLASS];
    float prov_Xi[TOTAL_FEATURE_VECTOR];
    ...
    for(int classType=1 ; classType<=TOTAL_CLASS ; classType++)
        prov_Ci[classType-1] = getProvabolity(1, classType);

    for(int attType=1 ; attType<=TOTAL_FEATURE_VECTOR ; attType++)
        prov_Xi[attType-1] = getProvabolity(attributeNo, attType);

    //for(int classType=1 ; classType<=TOTAL_CLASS ; classType++) cout << prov_Ci[classType-1] << endl;
    for(int attType=1 ; attType<=TOTAL_FEATURE_VECTOR ; attType++) cout << prov_Xi[attType-1] << endl;

    /*for(int classType=1 ; classType<=TOTAL_CLASS ; classType++) {
        for(int attType=1 ; attType<=TOTAL_FEATURE_VECTOR ; attType++) {

            prov_Ci_Xi = getProvability_with_class(classType, attributeNo, attType);
            cout << prov_Ci_Xi endl;
